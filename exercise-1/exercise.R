...
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = price, y = carat), color = 'darkblue', alpha = 0.5) +
  geom_line(mapping = aes(x = price, y = carat), color = 'blue')

# multiple geoms (point & smooth)
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = price, y = carat), color = 'darkblue') +
  geom_smooth(mapping = aes(x = price, y = carat), color = 'blue')
...
avg.price.by.clarity <- group_by(diamonds, clarity) %>%
  summarise(avg_price = mean(price))

ggplot(data = avg.price.by.clarity) +
  geom_bar(mapping = aes(x = avg_price, y = clarity))
